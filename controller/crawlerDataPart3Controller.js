...
export const crawlerDataPart3 = async (req, res) => {
  request('https://vnexpress.net/kinh-doanh', (error, respone, html) => {
...
export const crawlerDataPart3NextStep = async (req, res) => {
  request('https://vnexpress.net/giai-tri', (error, respone, html) => {
    if ((!error, respone?.statusCode == 200)) {
      const $ = cheerio.load(html);
      const load = $('.item-news');
      let data_Result = [];
